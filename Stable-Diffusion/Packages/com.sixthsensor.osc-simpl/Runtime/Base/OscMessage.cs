/*
	...
	Copyright 2016-2022 Sixth Sensor.
	All rights reserved.
	http://sixthsensor.dk
*/
...
namespace OscSimpl
{
	...
	public class OscMessage : OscPacket
	{
		...
		public void RemoveAt( int index )
		{
			...
			// Get args to remove.
			OscArgInfo argToRemoveInfo = _argsInfos[ index ];

			// Remove from args by shifting subsequent args one index down.
			for( int i = index; i < _argsCount-1; i++ ) _argsInfos[ i ] = _argsInfos[ i+1 ];
		...
		public bool TryGetArgTag( int index, out char tag )
		{
			...
			// Get.
			tag = (char) _argsInfos[index].tagByte;
			return true;
		}
		...
		bool TryGetArgSize( int index, out int size )
		{
			...
			// Get.
			size = _argsInfos[index].byteCount;
			return true;
		}
		...
		public bool TryGet( int index, out float value )
		{
			...
			// Get.
			int dataStartIndex = _argsInfos[ index ].byteIndex;
		...
		public bool TryGet( int index, out bool value )
		{
			...
			// Get.
			value = _argsInfos[index].tagByte == OscConst.tagTrueByte;
			return true;
		}
		...
		public OscMessage Set( int index, byte[] value )
		{
			...
			if( !BlobOscData.TryWriteTo( value, _argsData, ref dataStartIndex ) ) {
				Debug.Log( "input blob: " + value.Length + ", byteIndex: " + info.byteIndex + ", byteCount: " + info.byteCount + ", dataStartIndex: " + dataStartIndex );
				if( OscGlobals.logWarnings ) Debug.LogWarning( OscDebug.FailedWritingBytesWarning( this ) );
			}
			return this;
		}
		...
		public override bool TryWriteTo( byte[] data, ref int index )
		{
			...
			// Argument tags.
			for( int i = 0; i < _argsCount; i++ ) data[index++] = _argsInfos[i].tagByte;
		...
		public static bool TryReadFrom( byte[] sourceData, ref int sourceIndex, ref OscMessage message )
		{
			...
			// Adapt info array.
			if( message._argsInfos == null || message._argsInfos.Length < argCount ) message._argsInfos = new OscArgInfo[ argCount ];
			...
			for( int i = 0; i < argCount; i++ )
			{
				...
				message._argsInfos[ i ] = new OscArgInfo( tagByte, argType, ( short) argByteCount, (short) newArgsByteCount );
		...
		int AdaptiveSet( int index, OscArgInfo newArginfo )
		{
			...
			OscArgInfo oldArgInfo = expandArgs ? OscArgInfo.undefinedInfo : _argsInfos[ index ];
			...
			if( expandArgs )
			{
				...
				if( _argsInfos == null ) {
					_argsInfos = new OscArgInfo[ requiredArgCount ];
				} else if( requiredArgCount > _argsInfos.Length ) {
					OscArgInfo[] tempArgsInfo = new OscArgInfo[ requiredArgCount ];
					for( int i = 0; i < _argsInfos.Length; i++ ) tempArgsInfo[ i ] = _argsInfos[ i ];
					_argsInfos = tempArgsInfo;
				}

				// Insert nulls if needed.
				for( int i = _argsCount; i < requiredArgCount-1; i++ ) _argsInfos[ i ] = OscArgInfo.nullInfo; // Insert nulls if needed.

				// Place new arg and update count.
				_argsInfos[ requiredArgCount - 1 ] =  newArginfo;
				_argsCount = requiredArgCount;

			} else {
				// Overwite.
				_argsInfos[ index ] = newArginfo;
			}
		...
		bool ValidateTryGet( int argIndex, OscArgType requestedArgType )
		{
			const string failedPrependText = "TryGet failed. ";

			// Arg bounds.
			if( argIndex < 0 || argIndex >= _argsCount ) {
				if( OscGlobals.logWarnings ) {
					Debug.LogWarning(
						OscDebug.BuildText( this ).Append( failedPrependText )
						.Append( "Argument index " ).AppendGarbageFree( argIndex )
			...
			// Arg type.
			OscArgInfo argsInfo = _argsInfos[ argIndex ];
			if( requestedArgType != argsInfo.argType ){
				if( OscGlobals.logWarnings ) {
					Debug.LogWarning(
						OscDebug.BuildText( this ).Append( failedPrependText )
						.Append( "Argument at index " ).AppendGarbageFree( argIndex )
						.Append( " is not type " ).Append( requestedArgType )
						.Append( " '" ).Append( (char) OscConverter.ToTagByte( requestedArgType ) ).Append( "'" )
						.Append( ", it is type " ).Append( argsInfo.argType )
						.Append( " '" ).Append( (char) argsInfo.tagByte ).Append( "'.\n" )
					);
				}
				return false;
			}
			...
			// Data capacity.
			if( argsInfo.byteIndex + argsInfo.byteCount > _argsByteCount ) {
				if( OscGlobals.logWarnings ) {
					Debug.LogWarning(
						OscDebug.BuildText( this ).Append( failedPrependText )
						.Append( "Argument at index " ).AppendGarbageFree( argIndex ).Append( " has incomplete data.\n" )
					);
				}
				return false;
			}
